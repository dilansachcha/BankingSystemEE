...
| --- | --- | --- |
| **Authentication & Registration** |  | **Flow:** Users register with an `INACTIVE`/`PENDING` status. Passwords are mathematically salted and hashed via BCrypt. Secure JWT login is blocked until an Admin manually approves the account, which triggers an automated JavaMail notification. |
| **Customer Dashboard** |  | **Flow:** The central hub utilizing RxJS observables to fetch real-time aggregated balances. Displays active Checking, Savings, and Fixed Deposit accounts with visual status indicators and quick-action routing. |
| **Account Creation & PayHere Gateway** |  | **Flow:** When opening an account requiring an initial deposit, the backend creates a `PENDING` account, generates an MD5 cryptographic hash, and launches the PayHere modal. An asynchronous webhook securely flips the account to `ACTIVE` upon payment confirmation. |
| **Manual Fund Transfers** |  | **Flow:** Immediate inter-account transfers governed by strict JTA rollback validations. Enforces minimum balance requirements and prevents overdrafts during concurrent database write requests via row-level locking. |
| **Scheduled & Recurring Transfers** |  | **Flow:** Users configure one-time or recurring (daily, weekly, monthly) transfers. An EJB `@Schedule` polling bean executes these asynchronously, utilizing a cron-based 3-attempt retry logic for failed executions due to insufficient funds. |
| **Transaction History & PDF Export** |  | **Flow:** Paginated and sortable ledger of all credits and debits. Users can filter by date/account and instantly generate and download standard-compliant PDF transaction receipts. |
| **Admin Dashboard & Live Alerts** |  | **Flow:** The control center. Uses a persistent WebSocket connection to push live, unrefreshed alerts directly to the Admin UI whenever a transfer exceeds Rs. 50,000 or a new user attempts registration. |
| **Admin AI Fraud Detection** |  | **Flow:** Admins trigger an AI audit on specific users. The backend aggregates the user's historical transaction data and prompts **Google Gemini / xAI**, generating a JSON-formatted risk score (0-100) and a plain-text analysis of spending anomalies. |
